...
class SpecDecodeWorker(LoRANotSupportedWorkerBase):
    ...
    * The scoring forward pass is done without an MQA kernel, which is
    ...
    def init_device(self) -> None:
        ...
        if self._enable_lm_head_weight_load:
            if get_pp_group().is_last_rank:
                ...
                data = {"target_lm_head_weight": target_lm_head_weight}
                # Send target_lm_head_weight to the first pp rank
                get_pp_group().send_tensor_dict(data, dst=self._driver_rank)
            else:
                # Receive target_lm_head_weight from the last pp rank
                data = get_pp_group().recv_tensor_dict(
                    src=get_pp_group().world_size - 1)
                target_lm_head_weight = data["target_lm_head_weight"]
    ...
    def _run_no_spec(self, execute_model_req: ExecuteModelRequest,
        ...
        # Sampler output is None except the last pp rank
