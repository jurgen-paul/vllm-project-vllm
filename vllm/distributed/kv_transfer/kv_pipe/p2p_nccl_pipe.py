...
from vllm.distributed.device_communicators.pynccl_wrapper import (NCCLLibrary, buffer_type, ncclDataTypeEnum, cudaStream_t, ncclComm_t)
...
class P2pNcclPipe:
    def __init__(
        ...
        self._listener_thread = threading.Thread(target=self._listen_for_requests, daemon=True)
    ...
    def _create_connect(self, remote_address: typing.Optional[str] = None):
        ...
        if remote_address not in self.socks:
            ...
            with torch.cuda.device(self.device):
                ...
                logger.info(f"ncclCommInitRank Success, {self.local_address} 👉 {remote_address}, MyRank: {rank}")
    ...
    def send_tensor(
        ...
        if remote_address is None:
            ...
            data = {"cmd": "PUT", "tensor_id": tensor_id, "shape": tensor.shape, "dtype": tensor.dtype}
            sock.send(pickle.dumps(data))

            response = sock.recv()
            if response == b"0" and tensor is not None:
                self._send(comm, tensor.to(self.device), rank ^ 1)
                logger.info(f"Send Tensor Success, {self.local_address} 👉 {remote_address}, MyRank: {rank}, {data=}, {tensor=}")
    ...
    def recv_tensor(
        ...
        if tensor_id in self.store:
            ...
            # logger.debug(f"Rank {remote_address} received tensor")
        ...
        if data.ret == 0:
            tensor = torch.empty(data.shape, dtype=data.dtype, device=self.device)
    ...
    def _listen_for_requests(self):
        while True:
            ...
            if self.router_socket in socks:
                ...
                data = pickle.loads(message)
                # logger.info(f"Received message from {remote_address.decode()}, {data=}")
                if data["cmd"] == "NEW":
                    ...
                    with torch.cuda.device(self.device):
                        ...
                        comm: ncclComm_t = self.nccl.ncclCommInitRank(2, unique_id, rank)
                        self.comms[remote_address] = (comm, rank)
                        logger.info(f"ncclCommInitRank Success, {self.local_address} 👈 {remote_address.decode()}, MyRank: {rank}")
                elif data["cmd"] == "PUT":
                    ...
                    self._recv(comm, tensor, rank ^ 1)
                    self.store[tensor_id] = tensor
                    logger.info(f"Recv Tensor Success, {self.local_address} 👈 {remote_address.decode()}, MyRank: {rank}, {data=}, {tensor=}")
                elif data["cmd"] == "GET":
                    tensor_id = data["tensor_id"]
                    if tensor_id in self.store:
                        data = {"ret": 0, "shape": self.store[tensor_id].shape, "dtype": self.store[tensor_id].dtype}
                    else:
                        data = {"ret": 1}
                    self.router_socket.send_multipart([remote_address, pickle.dumps(data)])
                    if data["ret"] == 0:
                        self._send(comm, self.store[tensor_id].to(self.device), rank ^ 1)
                else:
                    logger.info(f"Bug, Received message from {remote_address}, {data=}")
