# adapted from https://huggingface.co/Skywork/Skywork-R1V-38B/blob/main/modeling_skywork_vit.py
# --------------------------------------------------------
# SkyworkR1V
...
class SkyworkVisionPatchModel(nn.Module):
...
class SkyworkParallelAttention(nn.Module):
...
class SkyworkSdpaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def _init_attn(
        ...
        *,
        ...
        if (num_heads + num_dummy_heads) % tp_size == 0:
            return SkyworkParallelAttention(config,
                                           quant_config=quant_config,
                                           num_dummy_heads=num_dummy_heads,
                                           prefix=prefix)

        return SkyworkSdpaAttention(config, num_dummy_heads=num_dummy_heads)
    ...
    def __init__(
        ...
        *,
        ...
        self.layers = nn.ModuleList([
            SkyworkVisionEncoderLayer(config,
                                     quant_config,
                                     num_dummy_heads=num_dummy_heads,
...
class SkyworkVisionModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.embeddings = SkyworkVisionEmbeddings(config)
        self.encoder = SkyworkVisionEncoder(
            config=config,
            quant_config=quant_config,
